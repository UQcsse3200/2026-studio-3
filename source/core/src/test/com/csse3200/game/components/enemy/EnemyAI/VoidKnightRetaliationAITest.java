...
class VoidKnightRetaliationAITest {
    ...
    void shouldRestoreArmourAfterRetaliation() {
        ...
        EnemyIntent retaliation =
                ai.decide(createContext(72, 72, 0, 2));

        EnemyIntent restoration =
                ai.decide(createContext(72, 72, 0, 3));
    ...
    void shouldReturnToNormalAttackAfterRestoringArmour() {
        ...
        EnemyIntent normalAttack =
                ai.decide(createContext(72, 72, 5, 4));

        assertEquals(
                EnemyIntent.attack(BASE_ATTACK * 2),
                retaliation);

        assertEquals(
                EnemyIntent.defend(RESTORED_ARMOUR),
                restoration);

        assertEquals(
                EnemyIntent.attack(BASE_ATTACK),
                normalAttack);
    }
    ...
    void shouldRepeatMechanicAfterArmourIsBrokenAgain() {
        ...
        assertEquals(
                EnemyIntent.attack(BASE_ATTACK * 2),
                ai.decide(createContext(72, 72, 0, 1)));
        ...
        assertEquals(
                EnemyIntent.attack(BASE_ATTACK),
                ai.decide(createContext(72, 72, 5, 3)));

        // 再次破盾
        assertEquals(
                EnemyIntent.attack(BASE_ATTACK * 2),
                ai.decide(createContext(72, 72, 0, 4)));

        // 再次恢复盾
        assertEquals(
                EnemyIntent.defend(RESTORED_ARMOUR),
                ai.decide(createContext(72, 72, 0, 5)));
    }
