...
public class CardEffectHandler {
    ...
    public CardEffectHandler(CardEffectResolver effectResolver, CardService cardService,
                             BattleDeck battleDeck, PlayerEffectState playerEffectState) {
    ...
    public CardPlayResult playCard(CardPlayRequest request, Entity player) {
        ...
        if (maybeCard.isEmpty()) {
            return CardPlayResult.failure(
                    "Unknown card: " + request.cardID(), request.cardID(),
                    request.targetID(), battleDeck);
        }
                ...
                switch (effect.type()) {
                    ...
                    case POISON ->
                            stats.applyStatusEffect(
                                    new StatusEffect("poison", effect.value(),
                                            effect.duration()));
                    case VULNERABLE ->
                            stats.applyStatusEffect(
                                    new StatusEffect("vulnerable", effect.value(),
                                            effect.duration()));
                    default -> {
