...
public class RewardDisplay extends Displaying {
    ...
    public RewardDisplay(DisplayingRecord rec, RewardService rewardService, Entity player) {
        super(rec);
    ...
    private void claimOption(RewardOption option) {
        ...
        claimed = true;
        try {
            rewardService.claimReward(player, option);
            entity.getEvents().trigger(REWARD_CLAIMED_EVENT);
        } catch (UnsupportedOperationException e) {
            // CARD_UPGRADE not yet implemented — surface this rather than crashing silently
            claimed = false;
            // TODO: show a "not available yet" message instead of allowing retry
        }
    }
